...
def تجميد_التوصية_التلقائي(جدول_التوصيات, تاريخ_الجلسة, db_path=None):
    ...
    db_path = db_path or os.getenv("QV_REVIEW_DB_PATH", "data/qv_recommendation_review.sqlite3")
...
def مراجعة_نتائج_جلسة_توصيات(تاريخ_المراجعة, بيانات_السوق, بيانات_لحظية=None, db_path=None):
    """مراجعة آلية لتوصيات الجلسة المجمدة وحساب نسبة النجاح."""
    if not QV_REVIEW_ENGINE_AVAILABLE:
        return {"جاهز": False, "الحالة": "محرك المراجعة غير متاح", "السبب": _qv_review_exc_text}
    db_path = db_path or os.getenv("QV_REVIEW_DB_PATH", "data/qv_recommendation_review.sqlite3")
    return QVRecommendationReviewEngine(db_path).review_actual_session(تاريخ_المراجعة, بيانات_السوق, بيانات_لحظية)
...
# مراجعة تلقائية: عند وصول جلسة سوق مقبولة، تُراجع فورًا التوصيات التي كان هذا اليوم موعد مراجعتها.
# لا تُستخدم نتائج الجلسة الحالية لإعادة بناء توصيات سابقة؛ المراجعة تقرأ دفترًا مجمدًا فقط.
try:
    _review_db = os.getenv("QV_REVIEW_DB_PATH", "data/qv_recommendation_review.sqlite3")
    _market = {}
    _intraday = {}
    for _row in df.to_dict(orient="records"):
        _sym = str(_row.get("الرمز", _row.get("symbol", _row.get("Symbol", ""))) or "").strip().upper()
        if not _sym: continue
        def _pick_num(names):
            for _n in names:
                if _n in _row and _row[_n] not in (None, "", "nan"):
                    try: return float(str(_row[_n]).replace(",", ""))
                    except Exception: pass
            return None
        _o=_pick_num(["open","Open","الافتتاح","سعر الافتتاح"])
        _h=_pick_num(["high","High","الأعلى","أعلى سعر"])
        _l=_pick_num(["low","Low","الأدنى","أدنى سعر"])
        _c=_pick_num(["close","Close","الإغلاق","سعر الإغلاق","السعر المرجعي"])
        if _h is not None and _l is not None and _c is not None:
            _market[_sym] = {"open": _o if _o is not None else _l, "high": _h, "low": _l, "close": _c}
    if _market:
        _auto_review = مراجعة_نتائج_جلسة_توصيات(حارس_الجلسة["تاريخ الجلسة"], _market, _intraday, _review_db)
        st.session_state["QV_AUTO_REVIEW_RESULT_" + str(حارس_الجلسة["تاريخ الجلسة"])] = _auto_review
except Exception as _review_exc:
    st.session_state["QV_AUTO_REVIEW_ERROR"] = str(_review_exc)
...
# ===================== Champion / Challenger OOS — 11.8 =====================
try:
    from qv_champion_challenger_engine import QVChampionChallengerEngine
    QV_CC_AVAILABLE=True; _qv_cc_exc_text=""
except Exception as _qv_cc_exc:
    QV_CC_AVAILABLE=False; _qv_cc_exc_text=str(_qv_cc_exc)

def حالة_Champion_Challenger_QV(db_path=None):
    if not QV_CC_AVAILABLE: return {"جاهز":False,"السبب":_qv_cc_exc_text}
    return QVChampionChallengerEngine(db_path or os.getenv("QV_REVIEW_DB_PATH","data/qv_recommendation_review.sqlite3"), min_oos=30).status()

def مقارنة_Champion_Challenger_QV(champion_id, challenger_id, تاريخ_حتى=None, db_path=None):
    if not QV_CC_AVAILABLE: return {"جاهزة":False,"السبب":_qv_cc_exc_text}
    return QVChampionChallengerEngine(db_path or os.getenv("QV_REVIEW_DB_PATH","data/qv_recommendation_review.sqlite3"), min_oos=30).compare(champion_id, challenger_id, تاريخ_حتى)

# ===================== ربط نتائج التوصيات بالمعايرة OOS — 11.7 =====================
def بناء_بيانات_معايرة_QV_من_دفتر_المراجعة(تاريخ_حتى=None, db_path=None):
    if not QV_REVIEW_ENGINE_AVAILABLE:
        return {"جاهزة":False,"السبب":"محرك المراجعة غير متاح"}
    db_path = db_path or os.getenv("QV_REVIEW_DB_PATH", "data/qv_recommendation_review.sqlite3")
    eng=QVRecommendationReviewEngine(db_path)
    return eng.calibration_ready_dataset(تاريخ_حتى, minimum=30)

def قياس_أداء_QV_التراكمي_من_النتائج(تاريخ_حتى=None, db_path=None):
    if not QV_REVIEW_ENGINE_AVAILABLE:
        return {"جاهز":False,"السبب":"محرك المراجعة غير متاح"}
    db_path = db_path or os.getenv("QV_REVIEW_DB_PATH", "data/qv_recommendation_review.sqlite3")
    eng=QVRecommendationReviewEngine(db_path)
    return eng.الأداء_حسب_المصدر_والنوع(تاريخ_حتى)

def معايرة_QV_المثبتة_من_الدفتر(تاريخ_حتى=None, db_path=None):
    data=بناء_بيانات_معايرة_QV_من_دفتر_المراجعة(تاريخ_حتى,db_path)
    if not data.get("execution_ready"):
        return {"جاهزة":False,"السبب":"لم تصل 30 حالة تنفيذ OOS مكتملة بعد","البيانات":data}
    # Reuse the existing strict OOS calibrator. No probability is emitted before its gate.
    cal=معايرة_احتمال_خارج_العينة(data["execution_scores"],data["execution_results"])
    cal["المصدر"]="دفتر التوصيات المجمد + نتائج جلسات لاحقة"
    cal["منع_التسرب"]=True
    return cal
...
try:
    ...
    if سجلات_الدورة:
        ...
        معايرة_الدورة = معايرة_QV_المثبتة_من_الدفتر(حارس_الجلسة.get("تاريخ الجلسة"))
        سلامة_الدورة = bool(تقرير_الدورة.get("المؤشرات", {}).get("جاهزية"))
        st.subheader("🎯 معايرة QV من نتائج التوصيات الفعلية")
        st.json({"حالة المعايرة": معايرة_الدورة})
        st.json({"الأداء حسب المصدر والاتجاه": قياس_أداء_QV_التراكمي_من_النتائج(حارس_الجلسة.get("تاريخ الجلسة"))})
...
st.divider()
with st.expander("🔄 المراجعة التلقائية للجلسة الحالية", expanded=False):
    _auto_key = "QV_AUTO_REVIEW_RESULT_" + str(حارس_الجلسة.get("تاريخ الجلسة"))
    _auto_result = st.session_state.get(_auto_key)
    if _auto_result:
        st.json(_auto_result)
    elif st.session_state.get("QV_AUTO_REVIEW_ERROR"):
        st.warning("تعذر تنفيذ المراجعة التلقائية: " + st.session_state["QV_AUTO_REVIEW_ERROR"])
    else:
        st.info("لا توجد مراجعة تلقائية متاحة لهذه الجلسة، أو لا توجد بيانات OHLC كافية.")
...
with st.expander("📊 محرك مراجعة نسبة نجاح توصيات كوانتوم فيكتوري", expanded=False):
    if not QV_REVIEW_ENGINE_AVAILABLE:
        ...
        review_engine = QVRecommendationReviewEngine(os.getenv("QV_REVIEW_DB_PATH", "data/qv_recommendation_review.sqlite3"))
        ...
        st.caption("سجل التدقيق يستخدم سلسلة هاش متتابعة؛ أي تعديل أو حذف داخل السجل يكسر التحقق.")
        st.subheader("📈 الأداء التراكمي")
        st.json(لوحة_المراجعة.get("global", {}))
        st.subheader("🔐 سلامة سجل التدقيق")
        try:
            st.json(review_engine.audit_integrity())
        except Exception as _audit_exc:
            st.warning("تعذر فحص سلامة سجل التدقيق: " + str(_audit_exc))



st.divider()
with st.expander("🏆 Champion / Challenger — مقارنة خارج العينة", expanded=False):
    if not QV_CC_AVAILABLE:
        st.error("محرك Champion / Challenger غير متاح: " + _qv_cc_exc_text)
    else:
        _cc=QVChampionChallengerEngine(os.getenv("QV_REVIEW_DB_PATH","data/qv_recommendation_review.sqlite3"), min_oos=30)
        st.json(_cc.status())
        st.caption("المقارنة OOS فقط. الحالات الملتبسة وغير المحسومة مستبعدة من النجاح التنفيذي. لا توجد ترقية تلقائية؛ الاعتماد المستقل يظل مطلوبًا.")
